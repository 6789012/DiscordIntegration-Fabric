...
import me.vetustus.server.simplechat.api.event.ChatMessageEvent;
...
public class DiscordIntegrationMod implements DedicatedServerModInitializer {
    ...
    public void onInitializeServer() {
		...
		ChatMessageEvent.EVENT.register(this::onChatMessageReceived);
	...
	public void onInitialize() {
            ...
            LOGGER.info("Message succecufuly send!!!");
			//DiscordIntegration.LOGGER.error("Chat was sent!!!");
            // Получаем сущность игрока
            //Entity sourceEntity = player;

            // Создаем текстовое сообщение из строки
            //String final_mesage = chatMessage.getMessage();

            // Обрабатываем сообщение чата с помощью Discord Integration
            //Text handledMessage = DiscordIntegrationMod.handleChatMessage(textMessage, player);

            // Отправляем сообщение в Discord
            
			//DiscordIntegration.INSTANCE.sendMessage("test", sourceEntity != null ? sourceEntity.getUuidAsString() : "0000000", new DiscordMessage(null, final_mesage, true), DiscordIntegration.INSTANCE.getChannel(Configuration.instance().advanced.chatOutputChannelID));
			
			
			//DiscordIntegration.INSTANCE.sendMessage("test", "0000000", new DiscordMessage(null, message, true), DiscordIntegration.INSTANCE.getChannel(Configuration.instance().advanced.chatOutputChannelID));
            // Если сообщение не было отменено, возвращаем его для дальнейшей обработки
            //if (!chatMessage.isCancelled()) {
               // chatMessage.setMessage(final_mesage);
            //}
	    ...
	    private void onChatMessageReceived(String playerName, String playerUUID, String message) {

        // Дополнительный код, который нужно выполнить
		LOGGER.info("Message succecufuly send!!!");
		message = removeBeforeColon(message);
		DiscordIntegration.INSTANCE.sendMessage(playerName, playerUUID, new DiscordMessage(null, message, true), DiscordIntegration.INSTANCE.getChannel(Configuration.instance().advanced.chatOutputChannelID));
    }
	
    public static String removeBeforeColon(String str) {
        int index = str.indexOf(':');
        if (index != -1) {
            return str.substring(index + 1);
        }
        return str;  // Если двоеточие не найдено, возвращаем исходную строку
    }
